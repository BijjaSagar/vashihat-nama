...
class SmartScanScreen extends StatefulWidget {
  final int userId;
  final String mode; // 'document' or 'credit_card'
  const SmartScanScreen({Key? key, required this.userId, this.mode = 'document'}) : super(key: key);
...
class _SmartScanScreenState extends State<SmartScanScreen> {
  ...
  Future<void> _processImage(File image) async {
    ...
    // Auto-classify using AI if text is found
    if (_recognizedText.isNotEmpty) {
       await _classifyWithAI(_recognizedText);
    } else {
       _analyzeText(_recognizedText); // Fallback to regex
    }
  ...
  Future<void> _classifyWithAI(String text) async {
    try {
      ...
      if (mounted) {
        setState(() {
          if (result.containsKey('category')) _docTypeController.text = result['category'];
          if (result.containsKey('title')) _notesController.text = result['title']; 
          // You could also extract dates from AI response if the backend prompt was updated to find dates.
          // For now, we still run the regex for dates as a backup or mix.
          _analyzeText(text); 
        });
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("AI Auto-Classified Document!")));
      }
    } catch (e) {
      // Fallback
      _analyzeText(text);
  ...
  void _analyzeText(String text) {
    ...
    // D. Detect Credit Card Number (13-19 digits with spaces/dashes)
    if (widget.mode == 'credit_card') {
      RegExp cardRegExp = RegExp(r"\b(?:\d[ -]*?){13,19}\b");
      Match? cardMatch = cardRegExp.firstMatch(text);
      if (cardMatch != null) {
        String cardNumber = cardMatch.group(0)!.replaceAll(RegExp(r'\D'), '');
        // Simple heuristic: if it looks like a card number, show it
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Detected Card: ${cardNumber.replaceAllMapped(RegExp(r".{4}"), (match) => "${match.group(0)} ")}"),
    ...
    // Auto-return if in card mode and we found a number
    if (widget.mode == 'credit_card') {
        RegExp cardRegExp = RegExp(r"\b(?:\d[ -]*?){13,19}\b");
        Match? cardMatch = cardRegExp.firstMatch(text);
        if (cardMatch != null) {
            ...
            // Try to find MM/YY
            RegExp expiryRegExp = RegExp(r"(\d{2})[\/](\d{2})");
            Match? expiryMatch = expiryRegExp.firstMatch(text);
            String? month, year;
            if (expiryMatch != null) {
                month = expiryMatch.group(1);
                year = expiryMatch.group(2);
            }
